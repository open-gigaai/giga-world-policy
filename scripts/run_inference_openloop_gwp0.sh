#!/usr/bin/env bash
# Open-loop eval for GWP0.
# Usage:
#   CHECKPOINT=<path> NORM_STATS=<path> DATA_PATH=<path> ./run_inference_openloop_gwp0.sh server
#   CHECKPOINT=<path> NORM_STATS=<path> DATA_PATH=<path> ./run_inference_openloop_gwp0.sh client
...
case "${1:-}" in
    server)
        ...
        shift
        python "${SCRIPT_DIR}/inference_openloop_gwp0.py" --server "${ARGS[@]}" "$@"
        ;;
    client)
        shift
        python "${SCRIPT_DIR}/inference_openloop_gwp0.py" "${ARGS[@]}" --replan-steps "${REPLAN_STEPS}" "$@"
        ;;
    *)
