...
def get_policy(
    ...
    # The server receives observations from the client over the socket. Loading
    # the LeRobot root here would require a giga_datasets config.json and is not
    # needed for open-loop inference.
    dataset = None
    replay_action = False
    replay_index = 0
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="torch.compile mode. reduce-overhead enables compiler-managed CUDA graph/replay when possible.",
    )
    parser.add_argument(
        "--compile-fullgraph",
        action="store_true",
        help="Pass fullgraph=True to torch.compile for debugging stricter graph capture.",
    )
