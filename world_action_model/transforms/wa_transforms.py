...
import numpy as np
import torch
import torch.nn.functional as torch_F  # 明确命名
...
class WATransforms:
    def __init__(
        ...
        if default_prompt_embeds_path is None:
            self.default_prompt_embeds = None
        else:
            self.default_prompt_embeds = torch.load(default_prompt_embeds_path)['condition_dict']['prompt_embeds'].squeeze(0)
        self.num_views = num_views

        if norm_path is None:
            raise ValueError("WATransformsV2 requires `norm_path` to be set in the config " "(JSON file containing norm_stats for state/action).")
        json_path = norm_path
    ...
    def __call__(self, data_dict):
        ...
        if self.num_views == 1:
            ...
            # front_view_images = DecordVideoReader(data_dict['front_video_path'])
        ...
        # state_mean = np.array(self.stats_dict['norm_stats']['observation.state' ]['mean'])
        # state_std = np.array(self.stats_dict['norm_stats']['observation.state' ]['std'])
        # delta_mean =   np.array(self.stats_dict['norm_stats']['action' ]['mean'])
        # delta_std =   np.array(self.stats_dict['norm_stats']['action']['std'])

        # norm_state = (state - state_mean) / state_std
        # norm_delta = (delta - delta_mean) / delta_std
        ...
        norm_action = (action - delta_min) / delta_range
        norm_state = norm_state * 2.0 - 1.0
        norm_delta = norm_delta * 2.0 - 1.0
        norm_action = norm_action * 2.0 - 1.0
        ...
        if self.is_train:
            ...
            if 'input_ref_images' in data_dict:
                ...
                prompt = data_dict["t5_embedding"][:32]
                new_data_dict["prompt_embeds"] = torch_F.pad(prompt, (0, 0, 0, 32 - prompt.shape[0]), value=0)
                # new_data_dict['prompt_embeds'] = torch_F.pad(
                #     data_dict['t5_embedding'],
                #     (0, 0, 0, max(0, 64 - data_dict['t5_embedding'].shape[0])),
                #     value=0
                # )
                if self.use_delta:
